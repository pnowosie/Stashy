namespace StashyLib
{
    ...
    public class Nuid 
    {
        ...
        public static Nuid NewNuid()
        {
            var g = Guid.NewGuid();
            return new Nuid(g.GetHashCode().ToString());
        }
    ...
    public partial class NuidyFileStashy : IStashy<Nuid>
    {
        public Nuid GetNewId<T1>() where T1 : new()
        {
            return Nuid.NewNuid();
        }

        public void Save<T1>(T1 t1, Nuid gId)
        {
        ...
        public T1 Load<T1>(Nuid gId) where T1 : new()
        {
            var xmlFileName = Path.Combine(GetObjectPath<T1>(), gId.ToString());
            EnsurePathExists(Path.GetDirectoryName(xmlFileName));
        ...
        public void Delete<T1>(Nuid gId)
        {
            var xmlFileName = Path.Combine(GetObjectPath<T1>(), gId.ToString());
            EnsurePathExists(Path.GetDirectoryName(xmlFileName));
